...
if ( ! defined('GIVE_PDF_PLUGIN_VERSION')) {
    define('GIVE_PDF_PLUGIN_VERSION', '3.2.2');
}

if ( ! defined('GIVE_PDF_MIN_GIVE_VERSION')) {
    define('GIVE_PDF_MIN_GIVE_VERSION', '3.3.0');
}
...
require_once trailingslashit(GIVE_PDF_PLUGIN_DIR) . 'vendor/autoload.php';
...
if ( ! class_exists('Give_PDF_Receipts')) :
    ...
    final class Give_PDF_Receipts
    {
         ...
         * @since 3.2.2
         *
         * @var Give_PDF_Receipts_Engine $engine Give_PDF_Receipts_Engine class object.
         */
        public $engine;
         ...
         * @since 3.2.0 Fix: flush rewrite rules on plugin activation
         *
         * @since  1.1.1
         */
        public function init_hooks()
        {
            ...
            register_activation_hook(GIVE_PDF_PLUGIN_FILE, [$this, 'flush_rewrite_rules']);
            register_deactivation_hook(GIVE_PDF_PLUGIN_FILE, [$this, 'deactivation']);
        }
        ...
        {
            add_filter('plugin_action_links_' . GIVE_PDF_PLUGIN_BASENAME, [$this, 'plugin_action_links']);
            add_filter('plugin_row_meta', [$this, 'plugin_row_meta'], 10, 2);
            add_filter('give-settings_get_settings_pages', [$this, 'add_settings'], 10, 1);
        }
        ...
        public static function flush_rewrite_rules() {
            self::give_pdf_register_post_type();
            flush_rewrite_rules();
        }
        ...
        public function check_environment()
        {
            ...
            if (empty($is_give_active)) {
                ...
                $this->add_admin_notice('prompt_give_activate', 'error',
                    sprintf(__('<strong>Activation Error:</strong> You must have the <a href="%s" target="_blank">GiveWP</a> plugin installed and activated for Give - PDF Receipts to activate.',
                        'give-pdf-receipts'), 'https://givewp.com'));
                $is_working = false;
            }
        ...
        public function get_environment_warning()
        {
            ...
            if (
                ...
                $this->add_admin_notice('prompt_give_incompatible', 'error',
                    sprintf(__('<strong>Activation Error:</strong> You must have the <a href="%s" target="_blank">GiveWP</a> core version %s for the Give - PDF Receipts add-on to activate.',
                        'give-pdf-receipts'), 'https://givewp.com', GIVE_PDF_MIN_GIVE_VERSION));
...
add_action(
    ...
    function () {
        ...
        if (give_pdf_receipts()->check_environment()) {
            give()->registerServiceProvider(GivePdfReceipt\PdfExport\ServiceProvider::class);
            give()->registerServiceProvider(GivePdfReceipt\FormExtension\ServiceProvider::class);
        }
    }
);
