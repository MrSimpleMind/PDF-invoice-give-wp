...
class Settings{
     ...
     *
     * @since 3.1.0 Add default empty params for id, name, and content
     * @since 2.4.0 Prevent overriding global template name
     *
     * @param int $formId
     */
    public static function SaveCustomTemplate($formId, $templateId = '', $templateName = '', $templateContent = '')
    {
		// Get request values
        $templateId = $_POST['give_pdf_receipt_template'] ?? $templateId;
        $templateName = isset($_POST['give_pdf_receipt_template_name']) ? give_clean($_POST['give_pdf_receipt_template_name']) : $templateName;
        $templateContent = isset($_POST['give_pdf_builder']) ? wp_kses_post($_POST['give_pdf_builder']) : $templateContent;
