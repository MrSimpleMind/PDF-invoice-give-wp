 ...
 * Supported PDF Tags.
 *
 * @since 3.1.0 Add param to return result as array
 * @since 2.4.0 Add written amount tags and donor_comment tag
 *
 * @return string|array
 */
function get_supported_pdf_tags($asArray = false)
{

      $intlExtensionNotice = !class_exists(NumberFormatter::class) ? ' (<a href="https://docs.givewp.com/currency-format">' .
      _x('requires INTL extension.', 'give-pdf-receipts') . '</a>)' : '';
	...
	$pdf_tags = array(
        ...
        'written_amount'  => _x( 'The written total price of the donation.', 'An explanation of the {written amount} template tag', 'give-pdf-receipts' ) . $intlExtensionNotice,
        'written_amount_without_decimals'  => _x( 'The written total price of the donation without decimals.', 'An explanation of the {written_amount_without_decimals} template tag', 'give-pdf-receipts' ). $intlExtensionNotice,
        'written_amount_dictated' => _x( 'The written total price of the donation dictated.', 'An explanation of the {written_amount_dictated} template tag', 'give-pdf-receipts' ). $intlExtensionNotice,
        'written_amount_dictated_without_decimals' => _x( 'The written total price of the donation dictated without decimals.', 'An explanation of the {written_amount_dictated_without_decimals} template tag', 'give-pdf-receipts' ). $intlExtensionNotice,
		...
		'donor_comment'   => _x( 'The Donor Comment that was submitted with the donation.', 'An explanation of the {donor_comment} template tag', 'give-pdf-receipts' ),
	);
 ...
 *
 * @since 2.4.0 If form, load the form template name instead of the global template name
 *
 * @param $value
 */
function give_pdf_receipt_template_name( $value ) {
    $single_give_form_id = isset($_GET['post']) ? give_clean($_GET['post']) : '';
    $template_id = ! empty($single_give_form_id) ? give_get_meta($single_give_form_id, 'give_pdf_receipt_template',
        true) : give_get_option('give_pdf_receipt_template');

    $template_name = is_string(get_post_status($template_id)) ? get_the_title($template_id) : __('Donation Receipt 1',
        'give-pdf-receipts');


    $screen = get_current_screen();

    if ($single_give_form_id) {
        $form_template_name = Give()->form_meta->get_meta($single_give_form_id, 'give_pdf_receipt_template_name', true);
        $template_name = ! empty($form_template_name) ? $form_template_name : $template_name;
    }

    ob_start(); ?>
    <input id="<?php
    echo esc_attr($value['id']); ?>" name="<?php
    echo esc_attr($value['id']); ?>" type="text" value="<?php
    echo $template_name; ?>" class="give-input-field" readonly />

    <div class="give-pdf-action-wrap">
        <a href="#" class="give-pdf-action-rename"><?php
            esc_html_e('Rename', 'give-pdf-receipts'); ?></a>
        <span class="give-pdf-action-sep">|</span>
        <a href="#" class="give-pdf-action-delete"><?php
            esc_html_e('Delete', 'give-pdf-receipts'); ?></a>
    </div>
    <p class="give-field-description"><?php
        echo give_get_field_description($value); ?></p>
    <?php
        ...
        echo ! empty($value['wrapper_class']) ? 'class="' . $value['wrapper_class'] . '"' : '' ?>>
            <th scope="row" class="titledesc">
                ...
                echo esc_attr($value['id']); ?>"><?php
                    echo esc_attr($value['name']); ?></label>
            </th>
    ...
    <?php
    // Single post type field wrap.
    elseif (isset($screen->post_type) && 'give_forms' === $screen->post_type) : ?>
        <fieldset class="give-field-wrap <?php
        echo esc_attr($value['id']); ?>_field <?php
        echo esc_attr($value['wrapper_class']); ?>">
            <label for="<?php
            echo give_get_field_name($value); ?>"><?php
                echo wp_kses_post($value['name']); ?></label>
            <?php
            echo $field_content; ?>
        </fieldset>
    <?php
