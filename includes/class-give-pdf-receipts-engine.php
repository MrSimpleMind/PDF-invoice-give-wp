...
class Give_PDF_Receipts_Engine {
     ...
     *
     * @since 2.4.0 Load donor's company name on PDF templates
     * @since      1.0
     * @uses       HTML2PDF
     * @uses       wp_is_mobile()
     */
	public function generate_pdf_receipt($donation_id = false, $save_to = '') {

        if (isset($donation_id['give-action'])) {
            $donation_id = false;
            $save_to = '';
        }
		...
		$donation_id = ! empty( $get_data['donation_id'] ) ? absint( $get_data['donation_id'] ) : $donation_id;
		...
		} elseif ( empty($save_to) && ! wp_verify_nonce( $get_data['_give_hash'], "give-download-pdf-receipt-{$donation_id}" ) ) {
			return;
		}
		...
		if ( $is_per_form ) {
			...
			$template_id  = $give_options['give_pdf_receipt_template'] ?? false;
		}
		...
		if ( $template_id && isset( $give_options['give_pdf_generation_method'] ) && 'custom_pdf_builder' === $give_options['give_pdf_generation_method'] ) {
			...
			return give_pdf_get_compile_html(
				array(
     ...
     *
     * @since 3.2.2 Add renewal donations to list of allowed donations
	 * @since  1.0
	 *
