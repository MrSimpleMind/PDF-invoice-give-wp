 ...
 *
 * @since 3.1.0 Return custom_templates
 * @since 2.1
 */
function delete_customized_pdf_template() {
	...
	wp_send_json_success( [
        ...
        'custom_templates' => (new GetCustomTemplates())(),
	] );
}
...
/**
 * Return the custom template ID used by a Form and a full list of custom templates created by the user.
 * This is useful for updating the UI without updating the whole page after creating a new custom template.
 *
 * @since 3.1.0
 */
function get_custom_templates()
{
    ...
    check_admin_referer("give_can_read_custom_templates", 'nonce');

    wp_send_json_success([
        'form_custom_template_id' => $form_id ? give()->form_meta->get_meta($form_id, "give_pdf_receipt_template",
            true) : 'create_new',
        'custom_templates' => (new GetCustomTemplates())(),
    ]);
}

add_action('wp_ajax_get_custom_templates', 'get_custom_templates');
