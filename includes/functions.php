...
use Give\Donations\Models\Donation;if ( ! defined( 'ABSPATH' ) ) {
	exit;
}
...
function give_pdf_receipts_replace_ffm_tags( $template_content, $payment_id ) {
	 ...
	 * @since 2.4.0 The FFM Form Helper was introduced in FFM v2.0.0
	 */
	if( class_exists( '\GiveFormFieldManager\Helpers\Form') ) {
		$form_data = \GiveFormFieldManager\Helpers\Form::get_input_fields($form_id);
	} else {
...
* @since 2.4.0
 */
function give_pdf_receipts_file_path(int $donation_id, string $save_to, string $pdf_receipt_filename = ''){
    if ( empty($pdf_receipt_filename)){
        $donation = Donation::find($donation_id);
        $date = date_i18n( get_option( 'date_format' ), $donation->createdAt->getTimestamp() );
        $pdf_receipt_filename = sprintf(
            ( __( 'Donation #%s Receipt - %s.pdf', 'give-pdf-receipts' ) ),
            $donation_id,
            str_replace('--', '-', str_replace(['/', '\\', ' ', ',', ':'], '-', $date))
        );
    }

    return trailingslashit($save_to) . $pdf_receipt_filename;
}
