...
/**
 * @since 3.1.1 Cast numbers before use the umber formatter
 * @since      2.4.0
*/
function give_pdf_get_written_amount($amount, $currency, $decimals = true, $dictated = false) {

    $written_amount = '';

    if ( empty($amount) || !class_exists('NumberFormatter')) {
        return $written_amount;
    }

    $amount = give_sanitize_amount($amount, ['currency' => $currency]);

    if (!$decimals && strpos( $amount, '.' ) !== false) {
        $amount = explode('.', $amount)[0];
    }

    $amount_formatter = new NumberFormatter(get_locale(), NumberFormatter::SPELLOUT);

    if ($dictated) {
        for ($i = 0; $i < mb_strlen($amount); $i++) {
            $char = mb_substr($amount, $i, 1);
            $written_amount .= is_numeric($char) ? $amount_formatter->format((int)$char) . ' - ' : '';
        }
        $written_amount = rtrim($written_amount, '- ');
    } else {
        $written_amount = $amount_formatter->format((float)$amount);
    }
...
* @since 2.4.0
 */
function give_pdf_replace_written_amount_tags($amount, $template_content, $currency) {

     if ( !class_exists('NumberFormatter')) {
       return $template_content;
     }

    $template_content = str_replace( '{written_amount}', give_pdf_get_written_amount($amount, $currency), $template_content );
    $template_content = str_replace( '{written_amount_without_decimals}', give_pdf_get_written_amount($amount, $currency, false), $template_content );
    $template_content = str_replace( '{written_amount_dictated}', give_pdf_get_written_amount($amount, $currency, true, true), $template_content );
    $template_content = str_replace( '{written_amount_dictated_without_decimals}', give_pdf_get_written_amount($amount, $currency, false, true), $template_content );
 ...
 *
 * @since 3.1.1 Use the total raw value on the written amount tags even when Fee Recovery is active
 ...
 */
function give_pdf_get_compile_html( $args = array(), $save_to = '' ) {
	...
	if ( give_pdf_is_preview_mode() ) {
		$company_name = "Donor's Company Name.";
	}
	...
	// Replace tags.
    $template_content = give_pdf_replace_written_amount_tags($give_pdf_total_price, $template_content,
        give_get_payment_currency_code($donation_id));
    ...
    $template_content = str_replace( '{donor_comment}', give_email_donor_comment( array( 'payment_id' => $donation_id ) ), $template_content );
	...
	if ( empty( $template_content ) ) {
        ...
        if (!empty($save_to)) {
            $filePath = give_pdf_receipts_file_path($donation_id, $save_to);
            $give_pdf->Output( $filePath, 'F' );
            return $filePath;
        } else {
 ...
 *
 * @since 2.4.0 Load sample donor's company name on the preview
 * @since      2.1.0
 *
...
function give_set_pdf_receipt_template_preview() {
    ...
    $company_name = __("Donor's Company Name.", 'give-pdf-receipts');
 ...
 * @return string
 * @since 2.2.1
 * @since 2.4.0 Remove colon before zipcode
 *
 */
function give_pdf_get_formatted_billing_address( $address ) {
	...
	// Replace newline character with <br> to make address PDF receipt compatible.
