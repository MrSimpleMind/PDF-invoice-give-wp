...
function give_pdf_template_default( $give_pdf, $give_pdf_payment, $give_pdf_payment_meta, $give_pdf_buyer_info, $give_pdf_payment_gateway, $give_pdf_payment_method, $address_line_2_line_height, $company_name, $give_pdf_payment_date, $give_pdf_payment_status ) {
    ...
    if (isset($give_options['give_pdf_logo_upload']) && ! empty($give_options['give_pdf_logo_upload'])) {
        ...
        $give_pdf->Image($give_options['give_pdf_logo_upload'], '', 30, '', '30', '', false, 'C', true, 300, 'C', false,
    ...
    if ( ! $fee_recovery_support) {
        ...
        } elseif ($word_count > 106 && $word_count <= 207) {
            // row 2.
            $rect_bottom_height = 30;
            $note_title_y = 180;
            $note_title_val_y = 188;
        } elseif ($word_count > 208 && $word_count <= 310) {
            // row 3.
            $rect_bottom_height = 35;
            $note_title_y = 186;
            $note_title_val_y = 196;
        } elseif ($word_count > 311) {
            // row 4.
    ...
    $give_pdf->SetTextColor(50, 50, 50);
    $give_pdf->Cell(0, 6, $give_pdf_buyer_info['first_name'] . ' ' . $give_pdf_buyer_info['last_name'], 0, 2, 'L',
        false);
