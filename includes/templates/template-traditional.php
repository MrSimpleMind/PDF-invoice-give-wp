...
function give_pdf_template_traditional( $give_pdf, $give_pdf_payment, $give_pdf_payment_meta, $give_pdf_buyer_info, $give_pdf_payment_gateway, $give_pdf_payment_method, $address_line_2_line_height, $company_name, $give_pdf_payment_date, $give_pdf_payment_status ) {
    ...
    $give_pdf->Cell(0, 60, apply_filters('give_pdf_receipt_heading', __('Donation Receipt', 'give-pdf-receipts')), 0, 0,
        'C', false);
    ...
    if (isset($give_options['give_pdf_logo_upload']) && ! empty($give_options['give_pdf_logo_upload'])) {
        ...
        $give_pdf->Cell(0, 0, $organization_name, 0, 0, 'C', false);
    }
    ...
    if ( ! empty($organization_name)) {
        $give_pdf->Cell(0, 6, $organization_name, 0, 2, 'C', false);
    }
    ...
    if ( ! empty($give_options['give_pdf_address_line2'])) {
        $give_pdf->Cell(0, give_pdf_calculate_line_height($give_options['give_pdf_address_line2']),
            give_pdf_get_settings($give_pdf, 'addr_line2'), 0, 2, 'C', false);
    }
    if ( ! empty($give_options['give_pdf_address_city_state_zip'])) {
        $give_pdf->Cell(0, give_pdf_calculate_line_height($give_options['give_pdf_address_city_state_zip']),
            give_pdf_get_settings($give_pdf, 'city_state_zip'), 0, 2, 'C', false);
    }

    $give_pdf->SetTextColor($rgb_code_array['r'], $rgb_code_array['g'], $rgb_code_array['b']);
    if ( ! empty($give_options['give_pdf_email_address'])) {
        $give_pdf->Cell(0, give_pdf_calculate_line_height($give_options['give_pdf_email_address']),
            give_pdf_get_settings($give_pdf, 'email'), 0, 2, 'C', false);
    }
