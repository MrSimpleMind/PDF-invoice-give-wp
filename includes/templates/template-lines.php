...
function give_pdf_template_lines( $give_pdf, $give_pdf_payment, $give_pdf_payment_meta, $give_pdf_buyer_info, $give_pdf_payment_gateway, $give_pdf_payment_method, $address_line_2_line_height, $company_name, $give_pdf_payment_date, $give_pdf_payment_status ) {
    ...
    $give_pdf->Cell(0, 0, apply_filters('give_pdf_receipt_heading', __('Donation Receipt', 'give-pdf-receipts')), 0, 0,
        'C', false);

    $organization_name = isset($give_options['give_pdf_company_name']) && ! empty($give_options['give_pdf_company_name']) ? $give_options['give_pdf_company_name'] : __('Organization Name',
        'give-pdf-receipts');

    // Set logo.
    if (isset($give_options['give_pdf_logo_upload']) && ! empty($give_options['give_pdf_logo_upload'])) {
        $give_pdf->setImageScale(2.25);
        $give_pdf->Image($give_options['give_pdf_logo_upload'], '', 20, '', '30', '', false, 'C', true, 300, 'C', false,
            false, 0, false, false, false);
    } else {
        $give_pdf->SetFont($currency_font, 'B', $font_size_32);
        $give_pdf->SetXY(10, 35);
        $give_pdf->Cell(0, 0, $organization_name, '', 0, 'C', false);
    }
    ...
    $give_pdf->Cell(0, 0, apply_filters('give_pdf_donation_date_title', __('Donation Date', 'give-pdf-receipts')), '',
        0, '', false);

    // Set Payment Method.
    $give_pdf->SetXY(65, 82);
    $give_pdf->SetFont($font, 'B', $font_size_12);
    $give_pdf->Cell(0, 0, apply_filters('give_pdf_payment_method_title', __('Payment Method', 'give-pdf-receipts')), '',
        0, '', false);

    // Set Payment ID.
    $give_pdf->SetXY(65, 89);
    $give_pdf->SetFont($font, 'B', $font_size_12);
    $give_pdf->Cell(0, 0, apply_filters('give_pdf_payment_id_title', __('Payment ID', 'give-pdf-receipts')), '', 0, '',
        false);

    // Set Donation Status.
    $give_pdf->SetXY(65, 96);
    $give_pdf->SetFont($font, 'B', $font_size_12);
    $give_pdf->Cell(0, 0, apply_filters('give_pdf_donation_status_title', __('Donation Status', 'give-pdf-receipts')),
        '', 0, '', false);
    ...
    $give_pdf->SetFont($currency_font, '', $font_size_12);
    $give_pdf->Cell(0, 0,
        ! empty($give_pdf_payment->ID) ? give_pdf_get_payment_number($give_pdf_payment->ID) : '123456789', 0, 2, 'L',
        false);
	...
	if ( $fee_recovery_support ) {
        ...
        $give_pdf->SetTextColor(50, 50, 50);
        $give_pdf->SetFont($currency_font, $currency_font_style, $font_size_14);
        $give_pdf->MultiCell(110, 7, $give_form_title, '0', 'L', false, 0, 26, 177, true, 0, false, true,
            apply_filters('give_pdf_form_title_max_height', 32), 'L', false);
    ...
    $give_pdf->SetTextColor(50, 50, 50);
    $give_pdf->Cell(0, give_pdf_calculate_line_height($give_pdf_buyer_info['first_name']),
        $give_pdf_buyer_info['first_name'] . ' ' . $give_pdf_buyer_info['last_name'], 0, 2, 'L', false);
    ...
    if ( ! empty($give_pdf_buyer_info['address'])) {
        ...
        if ( ! empty($give_pdf_buyer_info['address']['city'])
            ...
            $give_pdf->Cell(0, 6,
                $give_pdf_buyer_info['address']['city'] . ' ' . $give_pdf_buyer_info['address']['state'] . ' ' . $give_pdf_buyer_info['address']['zip'],
                0, 2, 'L', false);
        }
    ...
    $give_pdf->SetFont($font, 'B', $font_size_12);
    $give_pdf->SetTextColor($rgb_code_array['r'], $rgb_code_array['g'], $rgb_code_array['b']);
    $give_pdf->Cell(0, 0, apply_filters('give_pdf_organization_title', __('Organization', 'give-pdf-receipts')), '', 0,
        'L', false);
    ...
    if ( ! empty($organization_name)) {
        $give_pdf->Cell(0, 6, $organization_name, 0, 2, 'L', false);
    }
    ...
    if (isset($give_options['give_pdf_additional_notes']) && ! empty($give_options['give_pdf_additional_notes'])) {
        ...
        $give_pdf->SetFont($font, 'B', $font_size_12);
        $give_pdf->Cell(0, 12,
            apply_filters('give_pdf_additional_notes_title', __('Additional Notes', 'give-pdf-receipts')), '', 2, 'L',
            false);
