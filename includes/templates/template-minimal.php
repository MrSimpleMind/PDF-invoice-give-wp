...
function give_pdf_template_minimal( $give_pdf, $give_pdf_payment, $give_pdf_payment_meta, $give_pdf_buyer_info, $give_pdf_payment_gateway, $give_pdf_payment_method, $address_line_2_line_height, $company_name, $give_pdf_payment_date, $give_pdf_payment_status ) {
	...
	if ( $fee_recovery_support ) {
        ...
        $give_pdf->SetFont($font, 'B', $font_size_12);
        $give_pdf->SetTextColor($rgb_code_array['r'], $rgb_code_array['g'], $rgb_code_array['b']);
        $give_pdf->Cell(102, 0, apply_filters('give_pdf_donation_name_title', __('Donation Name', 'give-pdf-receipts')),
            '', '', 'L', false);

        // Set Donation amount.
        $give_pdf->SetXY(133, 169);
        $give_pdf->Cell(0, 0,
            apply_filters('give_pdf_donation_amount_title', __('Donation Amount', 'give-pdf-receipts')), '', '', 'L',
            false);

        $give_pdf->SetTextColor(50, 50, 50);
        $give_pdf->SetFont($currency_font, $currency_font_style, $font_size_14);
        $give_pdf->MultiCell(110, 7, $give_form_title, '0', '', false, 0, 20, 177, true, 0, false, true,
    ...
    $give_pdf->SetFont($font, 'B', $font_size_12);
    $give_pdf->SetTextColor($rgb_code_array['r'], $rgb_code_array['g'], $rgb_code_array['b']);
    $give_pdf->Cell(0, 12, apply_filters('give_pdf_organization_title', __('Organization', 'give-pdf-receipts')), '', 0,
        'L', false);
